...
(function ($) { 
	if ( $('div').hasClass('cmsmasters_items_filter_wrap')) {
		var el, leftPos, topPos, newTopPos, newWidth,
			mainNav = $('.cmsmasters_items_filter .cmsmasters_items_filter_list');
		...
		magicLine
			.width($('.current').width())
			.css('left', $('.current a').parent().position().left)
			.css('top', $('.current a').parent().height())
			.data('origLeft', magicLine.position().left)
			.data('origTop', magicLine.height())
			.data('origWidth', magicLine.width());
		
		$('.cmsmasters_items_filter .cmsmasters_items_filter_list li a').on('click', function () {
			el = $(this);
		...
		$(window).on('debouncedresize', function () {
			el = $('.cmsmasters_items_filter .cmsmasters_items_filter_list li.current a');
			el.each(function () {
				var el_new = $(this);
...
function startPortfolio(id, layout, layoutMode, url, orderby, order, count, categories, metadata) { 
	...
	portfolioContainer.find('.cmsmasters_project_sort_block .cmsmasters_project_sort_but').on('click', function () { 
		var type = jQuery(this).attr('name'), 
			...
			current = (jQuery(this).hasClass('active')) ? true : false, 
		...
		if (current) { 
			...
			if (type === 'project_name') { 
				...
				jQuery(this).addClass('active reversed');
			}
		}
