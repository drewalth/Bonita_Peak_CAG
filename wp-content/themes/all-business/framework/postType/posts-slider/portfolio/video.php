...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	...
	<?php 
		all_business_thumb_rollover(get_the_ID(), 'cmsmasters-single-project-thumb', false, false, true, false, $cmsmasters_project_video_type, $cmsmasters_project_video_link, $cmsmasters_project_video_links, false, true, $cmsmasters_project_link_redirect, $cmsmasters_project_link_url);
		
		
		if ($title || $categories || $comments || $likes) {
			echo '<div class="slider_project_inner">';
				
				($title) ? all_business_slider_post_heading(get_the_ID(), 'project', 'h2', $cmsmasters_project_link_redirect, $cmsmasters_project_link_url, true, $cmsmasters_project_link_target) : '';
