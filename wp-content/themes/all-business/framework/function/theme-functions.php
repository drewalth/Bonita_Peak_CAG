...
if (!defined('CMSMASTERS_SHORTNAME')) {
	define('CMSMASTERS_SHORTNAME', 'all-business');
}

if (!defined('CMSMASTERS_FULLNAME')) {
	define('CMSMASTERS_FULLNAME', 'All Business');
}
...
function register_css_styles() {
	if (!is_admin()) {
		...
		if (
			...
			wp_enqueue_style('theme-fonts-schemes', $cmsmasters_styles_dir . 'all-business' . '.css', array(), '1.0.0', 'screen');
		}
		
		
		wp_enqueue_style('theme-fonts', $cmsmasters_styles_dir . 'all-business' . '_fonts.css', array(), '1.0.0', 'screen');
		
		wp_enqueue_style('theme-schemes-primary', $cmsmasters_styles_dir . 'all-business' . '_colors_primary.css', array(), '1.0.0', 'screen');
		
		wp_enqueue_style('theme-schemes-secondary', $cmsmasters_styles_dir . 'all-business' . '_colors_secondary.css', array(), '1.0.0', 'screen');
...
function register_js_scripts() {
	if (!is_admin()) {
		...
		wp_register_script('ripple', get_template_directory_uri() . '/js/ripple-config.js', array('jquery'), '1.0.0', true);
		
		
		$primary_color = $cmsmasters_option[CMSMASTERS_SHORTNAME . '_default_link'];
		
		$secondary_color = $cmsmasters_option[CMSMASTERS_SHORTNAME . '_default_secondary'];
		...
		wp_localize_script('script', 'cmsmasters_script', array( 
			...
			'ilightbox_close_text' => 				esc_html__('Close', 'all-business'), 
			'ilightbox_enter_fullscreen_text' => 	esc_html__('Enter Fullscreen (Shift+Enter)', 'all-business'), 
			'ilightbox_exit_fullscreen_text' => 	esc_html__('Exit Fullscreen (Shift+Enter)', 'all-business'), 
			'ilightbox_slideshow_text' => 			esc_html__('Slideshow', 'all-business'), 
			'ilightbox_next_text' => 				esc_html__('Next', 'all-business'), 
			'ilightbox_previous_text' => 			esc_html__('Previous', 'all-business'), 
			'ilightbox_load_image_error' => 		esc_html__('An error occurred when trying to load photo.', 'all-business'), 
			'ilightbox_load_contents_error' => 		esc_html__('An error occurred when trying to load contents.', 'all-business'), 
			'ilightbox_missing_plugin_error' => 	esc_html__("The content your are attempting to view requires the <a href='{pluginspage}' target='_blank'>{type} plugin<\/a>.", 'all-business') 
		));
		...
		if ( 
			is_a($post, 'WP_Post') && 
			strpos($post->post_content, '[cmsmasters_button ') 
		) {
		...
		if (CMSMASTERS_WOOCOMMERCE) {
			...
			$cmsmasters_image_sizes = all_business_get_image_thumbnail_list();
...
/* Developer Mode */
function all_business_developer_mode() {
	if (!is_admin()) {
...
if (!function_exists('all_business_theme_google_fonts_generate')) {

function all_business_theme_google_fonts_generate() {
	$cmsmasters_option = all_business_get_global_options();
...
add_action('wp_enqueue_scripts', 'all_business_theme_google_fonts_generate');
...
function all_business_get_google_font($font) {
	if ($font != '') {
		if (strpos($font, ':')) {
...
function the_widgets_init() {
    ...
    register_sidebar(
        array(
            'name' => esc_html__('Sidebar', 'all-business'), 
            'id' => 'sidebar_default', 
            'description' => esc_html__('Widgets in this area will be shown in all left and right sidebars till you don\'t use custom sidebar.', 'all-business'), 
    ...
    register_sidebar(
        array(
            'name' => esc_html__('Bottom Sidebar', 'all-business'), 
            'id' => 'sidebar_bottom', 
            'description' => esc_html__('Widgets in this area will be shown at the bottom of middle block below the content and middle sidebar, but above footer.', 'all-business'), 
            'before_widget' => '<aside id="%1$s" class="widget %2$s">', 
            'after_widget' => '</aside>', 
            'before_title' => '<h3 class="widgettitle">', 
            'after_title' => '</h3>'
    ...
    register_sidebar(
        array(
            'name' => esc_html__('Archive Sidebar', 'all-business'), 
            'id' => 'sidebar_archive', 
            'description' => esc_html__('Widgets in this area will be shown in all left and right sidebars on archives pages.', 'all-business'), 
            'before_widget' => '<aside id="%1$s" class="widget %2$s">', 
            'after_widget' => '</aside>', 
            'before_title' => '<h3 class="widgettitle">', 
            'after_title' => '</h3>'
    ...
    register_sidebar(
        array(
            'name' => esc_html__('Search Sidebar', 'all-business'), 
            'id' => 'sidebar_search', 
            'description' => esc_html__('Widgets in this area will be shown in left or right sidebar on search page.', 'all-business'), 
            'before_widget' => '<aside id="%1$s" class="widget %2$s">', 
            'after_widget' => '</aside>', 
            'before_title' => '<h3 class="widgettitle">', 
            'after_title' => '</h3>'
	...
	if (isset($cmsmasters_option[CMSMASTERS_SHORTNAME . '_sidebar']) && sizeof($cmsmasters_option[CMSMASTERS_SHORTNAME . '_sidebar']) > 0) {
		foreach ($cmsmasters_option[CMSMASTERS_SHORTNAME . '_sidebar'] as $sidebar) {
			register_sidebar(array( 
				...
				'description' => esc_html__('Custom sidebar created with cmsmasters admin panel.', 'all-business'), 
				'before_widget' => '<aside id="%1$s" class="widget %2$s">', 
				'after_widget' => '</aside>', 
				'before_title' => '<h3 class="widgettitle">', 
				'after_title' => '</h3>' 
...
register_nav_menus(array( 
    'primary' => 	esc_html__('Primary Navigation', 'all-business'), 
    'footer' => 	esc_html__('Footer Navigation', 'all-business'), 
	'top_line' => 	esc_html__('Top Line Navigation', 'all-business') 
));



/* Register Showing Home Page on Default WordPress Pages Menu */
function all_business_page_menu_args($args) {
    $args['show_home'] = true;
...
add_filter('wp_page_menu_args', 'all_business_page_menu_args');
...
function all_business_fallback_menu($args) {
	$cmsmasters_option = all_business_get_global_options();
...
if (function_exists('add_theme_support')) {
	...
	function all_business_add_post_formats_to_project() {
		add_post_type_support('project', 'post-formats');
		
		register_taxonomy_for_object_type('post_format', 'project');
	}
	
	add_action('init', 'all_business_add_post_formats_to_project', 11);
	...
	add_filter('image_size_names_choose', 'all_business_select_image_size');
}



/* Add Image Thumbnails Size to the List */
function all_business_select_image_size($sizes) {
	$thumbnail_list = cmsmasters_image_thumbnail_list();
...
function all_business_add_editor_styles() {
...
add_action('init', 'all_business_add_editor_styles');
...
function all_business_post_author_archive($query) {
...
add_action('pre_get_posts', 'all_business_post_author_archive');



/* Check Row p Wrapper */
function all_business_rowcheck($content) {
...
function all_business_add_global_icons() {
...
function all_business_regenerate_styles() {
	$custom_css_fonts = all_business_theme_fonts();
	
	$custom_css_colors_primary = all_business_theme_colors_primary();
	
	$custom_css_colors_secondary = all_business_theme_colors_secondary();
	
	
	$custom_css = $custom_css_fonts . $custom_css_colors_primary . $custom_css_colors_secondary;
	
	
	all_business_write_styles($custom_css_fonts, CMSMASTERS_SHORTNAME . '_fonts');
	
	all_business_write_styles($custom_css_colors_primary, CMSMASTERS_SHORTNAME . '_colors_primary');
	
	all_business_write_styles($custom_css_colors_secondary, CMSMASTERS_SHORTNAME . '_colors_secondary');
	
	
	all_business_write_styles($custom_css);
}



/* Write Custom Styles to File Function */
function all_business_write_styles($styles, $filename = '') {
	$upload_dir = wp_upload_dir();
	...
	$is_dir = all_business_create_folder($style_dir);
	...
	$created = all_business_create_file($file, $styles);
...
function all_business_create_folder(&$folder, $addindex = true) {
...
function all_business_create_file($file, $content = '', $verifycontent = true) {
	global $wp_filesystem;
...
function cmsmasters_get_tweets($username, $count) {
	...
	load_template(get_template_directory() . '/' . CMSMASTERS_CLASS . '/OAuth.php', true);
...
function sidebarDefaultText() {
	...
	'<aside id="archives" class="widget widget_archive">' . "\n" . 
		'<h3 class="widgettitle">' . esc_html__('Archives', 'all-business') . '</h3>' . "\n" . 
		'<ul>' . "\n";
	...
	'<aside id="meta" class="widget widget_meta">' . "\n" . 
		'<h3 class="widgettitle">' . esc_html__('Meta', 'all-business') . '</h3>' . "\n" . 
		'<ul>' . "\n\t";
	...
	echo '" title="' . esc_attr__('Syndicate this site using RSS 2.0', 'all-business') . '">' . esc_html__('Entries', 'all-business') . ' ' . 
			'<abbr title="' . esc_attr__('Really Simple Syndication', 'all-business') . '">' . esc_html__('RSS', 'all-business') . '</abbr>' . 
		'</a>' . 
	'</li>' . "\n\t" . 
	'<li>' . 
		'<a href="';
	...
	echo '" title="' . esc_attr__('The latest comments to all posts in RSS', 'all-business') . '">' . esc_html__('Comments', 'all-business') . ' ' . 
					'<abbr title="' . esc_attr__('Really Simple Syndication', 'all-business') . '">' . esc_html__('RSS', 'all-business') . '</abbr>' . 
				'</a>' . 
			'</li>' . "\n\t" . 
			'<li>' . 
				'<a href="http://wordpress.org/" title="' . esc_attr__('Powered by WordPress, state-of-the-art semantic personal publishing platform.', 'all-business') . '">WordPress.org</a>' . 
			'</li>' . "\r" . 
		'</ul>' . "\n" . 
	'</aside>' . "\n";
}
...
function all_business_theme_header_styles() {
	$cmsmasters_option = all_business_get_global_options();
...
function all_business_theme_bg_styles() {
	global $post;
...
/* Theme Custom Styles */
function all_business_theme_custom_styles() {
	$out = '<style type="text/css">' . 
		all_business_theme_header_styles() . 
		all_business_theme_bg_styles() . 
		(CMSMASTERS_COLORED_CATEGORIES ? all_business_theme_category_styles() : '') . 
		all_business_theme_social_icons_styles() . 
	'</style>';
...
add_action('wp_head', 'all_business_theme_custom_styles');



/* Theme Footer Scripts Function */
function all_business_theme_footer_scripts() {
	$cmsmasters_option = all_business_get_global_options();
...
add_action('wp_print_footer_scripts', 'all_business_theme_footer_scripts');



/* Theme Page Layout and Scheme Function */
function all_business_theme_page_layout_scheme() {
	$cmsmasters_option = all_business_get_global_options();
...
function all_business_global_shortcodes_styles_move($content) {
...
add_filter('the_content', 'all_business_global_shortcodes_styles_move', 11, 2);
add_filter('widget_text_content', 'all_business_global_shortcodes_styles_move', 11, 3);


function all_business_row_checker($data) {
	$out = all_business_rowcheck($data);
	
	return $out;
}

add_filter('the_content', 'all_business_row_checker', 10, 2);



/* Project Post Type Registration Rename */
function all_business_project_labels() {
	return array( 
		'name' => 					esc_html__('Projects', 'all-business'), 
		'singular_name' => 			esc_html__('Project', 'all-business'), 
		'menu_name' => 				esc_html__('Projects', 'all-business'), 
		'all_items' => 				esc_html__('All Projects', 'all-business'), 
		'add_new' => 				esc_html__('Add New', 'all-business'), 
		'add_new_item' => 			esc_html__('Add New Project', 'all-business'), 
		'edit_item' => 				esc_html__('Edit Project', 'all-business'), 
		'new_item' => 				esc_html__('New Project', 'all-business'), 
		'view_item' => 				esc_html__('View Project', 'all-business'), 
		'search_items' => 			esc_html__('Search Projects', 'all-business'), 
		'not_found' => 				esc_html__('No projects found', 'all-business'), 
		'not_found_in_trash' => 	esc_html__('No projects found in Trash', 'all-business') 
	);
}

// add_filter('cmsmasters_project_labels_filter', 'all_business_project_labels');


function all_business_pj_categs_labels() {
	return array( 
		'name' => 					esc_html__('Project Categories', 'all-business'), 
		'singular_name' => 			esc_html__('Project Category', 'all-business') 
	);
}

// add_filter('cmsmasters_pj_categs_labels_filter', 'all_business_pj_categs_labels');


function all_business_pj_tags_labels() {
	return array( 
		'name' => 					esc_html__('Project Tags', 'all-business'), 
		'singular_name' => 			esc_html__('Project Tag', 'all-business') 
	);
}

// add_filter('cmsmasters_pj_tags_labels_filter', 'all_business_pj_tags_labels');



/* Profile Post Type Registration Rename */
function all_business_profile_labels() {
	return array( 
		'name' => 					esc_html__('Profiles', 'all-business'), 
		'singular_name' => 			esc_html__('Profiles', 'all-business'), 
		'menu_name' => 				esc_html__('Profiles', 'all-business'), 
		'all_items' => 				esc_html__('All Profiles', 'all-business'), 
		'add_new' => 				esc_html__('Add New', 'all-business'), 
		'add_new_item' => 			esc_html__('Add New Profile', 'all-business'), 
		'edit_item' => 				esc_html__('Edit Profile', 'all-business'), 
		'new_item' => 				esc_html__('New Profile', 'all-business'), 
		'view_item' => 				esc_html__('View Profile', 'all-business'), 
		'search_items' => 			esc_html__('Search Profiles', 'all-business'), 
		'not_found' => 				esc_html__('No Profiles found', 'all-business'), 
		'not_found_in_trash' => 	esc_html__('No Profiles found in Trash', 'all-business') 
	);
}

// add_filter('cmsmasters_profile_labels_filter', 'all_business_profile_labels');


function all_business_pl_categs_labels() {
	return array( 
		'name' => 					esc_html__('Profile Categories', 'all-business'), 
		'singular_name' => 			esc_html__('Profile Category', 'all-business') 
	);
}

// add_filter('cmsmasters_pl_categs_labels_filter', 'all_business_pl_categs_labels');
